...
* Some of the API state may actually end up being compiled into the shader.
...
* Then there’s commands that change state. GPU is a massively parallel computer, and you can’t just change a global variable in a parallel system and hope that everything works out OK
